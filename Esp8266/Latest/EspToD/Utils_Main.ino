...
void SendSensorData() {
  ...
  String content1 = String(millis()%6+1);
  String topic1 = String(WiFi.macAddress()) + "/dice";
  client.publish(topic1.c_str(), content1.c_str(), content.length());
#ifdef VerbosePublish
  Serial.println("Published: " + content);
  Serial.println("Published2: " + content1);
#endif
}

#define WIFI_SSID_FILE "/WiFi_SSID.txt"
#define WIFI_PASS_FILE "/WiFi_pass.txt"
#define WIFI_SSID_SERVER_INPUT_NAME "wifi_ssid"
#define WIFI_PASS_SERVER_INPUT_NAME "wifi_pass"

bool spiffsInitialized = false;
bool wasFactoryReseted = false;
bool sensorInitialized = false;
bool verboseSensor = false;
bool verboseHttpServer = true;
bool httpServerUp = false;
int bootAttempts = -1;
int lastFrameFPS = 0;
String deviceName="Range Sensor";

ulong timeOfLastAttemptToConnectToMqttServer = 0;
ulong minMqttConnectionAttemptTime = 500;

ulong* tolatctms = &timeOfLastAttemptToConnectToMqttServer;
ulong* mmcat = &minMqttConnectionAttemptTime;

ulong timeOfLastPublish = 0;
ulong minPublishTime = 2000;

ulong* tolf = &timeOfLastPublish;
ulong* mpt = &minPublishTime;

static const char serverCert[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF";

static const char serverKey[] PROGMEM = R"EOF(
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
)EOF";
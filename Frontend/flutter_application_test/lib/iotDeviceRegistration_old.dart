...
import 'API/iotDeviceAPI/apiConfig.dart';
...
class _IoTDeviceRegistrationWidgetState
  ...
  TextEditingController _httpServerIPandPortController =
      TextEditingController(text: "$iotDeviceIP:$port");
  ...
  Future<bool> _addNewIoTDevice() async {
    ...
    String? currNetworkHTTPServerMAC = await getIoTDeviceMAC();
    ...
    print(
        "Curr Network SSID: $currDeviceModel, Curr Gateway MAC: $currNetworkHTTPServerMAC");

    // append new device data to local variable _addedIoTDevices
    // update preferences variable _addedIoTDevices
    if (success == true &&
        currDeviceModel != null &&
        currNetworkHTTPServerMAC != null) {
      IoTDevice newIoTDevice = IoTDevice.unregistered(
          model: currDeviceModel, mac: currNetworkHTTPServerMAC);
